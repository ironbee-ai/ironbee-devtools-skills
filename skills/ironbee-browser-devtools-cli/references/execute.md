...
```bash
ironbee-browser-devtools-cli run execute (--code "<javascript>" | --file <path>) [options]
```
...
| `--file` | string | Yes* | - | Read script body from a UTF-8 file. Mutually exclusive with `--code`; provide exactly one. |
| `--timeout-ms` | number | No | `30000` | Wall-clock timeout for the entire execution in ms. Max: 120000. |

\*Exactly one of `--code` or `--file` is required.
...
# Custom timeout
ironbee-browser-devtools-cli run execute --code "await page.evaluate(() => 1+1);" --timeout-ms 10000

# Run script body from a file
ironbee-browser-devtools-cli run execute --file ./scripts/login-flow.js

# JSON output
ironbee-browser-devtools-cli --json run execute --code "return await page.title();"
```
