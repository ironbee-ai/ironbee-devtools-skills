...
```bash
ironbee-browser-devtools-cli sync wait-for-network-idle [options]
```
...
ironbee-browser-devtools-cli sync wait-for-network-idle --idle-time-ms 200

# Longer timeout for slow pages
ironbee-browser-devtools-cli sync wait-for-network-idle --timeout-ms 60000

# Stricter idle requirement
ironbee-browser-devtools-cli sync wait-for-network-idle --idle-time-ms 1000

# JSON output
ironbee-browser-devtools-cli --json sync wait-for-network-idle
```
...
ironbee-browser-devtools-cli sync wait-for-network-idle
ironbee-browser-devtools-cli content take-screenshot --name "loaded"
```

### After User Interaction

```bash
ironbee-browser-devtools-cli interaction click --selector "#submit"
ironbee-browser-devtools-cli sync wait-for-network-idle
...
```bash
ironbee-browser-devtools-cli interaction fill --selector "#search" --value "test"
ironbee-browser-devtools-cli interaction press-key --key "Enter"
ironbee-browser-devtools-cli sync wait-for-network-idle
ironbee-browser-devtools-cli content get-as-text --selector ".results"
```
...
ironbee-browser-devtools-cli $SESSION interaction fill --selector "#password" --value "password"
...
ironbee-browser-devtools-cli $SESSION content get-as-text
```
...
# Navigate
ironbee-browser-devtools-cli $SESSION navigation go-to --url "https://example.com/feed"
ironbee-browser-devtools-cli $SESSION sync wait-for-network-idle

# Scroll down to trigger load
ironbee-browser-devtools-cli $SESSION interaction scroll --mode by --dy 1000

# Wait for new content
ironbee-browser-devtools-cli $SESSION sync wait-for-network-idle --idle-time-ms 200

# Verify content (e.g. take screenshot or get-as-text)
ironbee-browser-devtools-cli $SESSION content take-screenshot --name "feed"
```
