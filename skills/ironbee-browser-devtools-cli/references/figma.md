...
```bash
ironbee-browser-devtools-cli figma compare-page-with-design --figma-file-key <key> --figma-node-id <id> [options]
```
...
ironbee-browser-devtools-cli --json figma compare-page-with-design \
  --figma-file-key "xxx" --figma-node-id "1:2" \
  --selector "#header"
...
ironbee-browser-devtools-cli $SESSION navigation go-to --url "http://localhost:3000/dashboard"
ironbee-browser-devtools-cli $SESSION sync wait-for-network-idle

# Compare header
HEADER_RESULT=$(ironbee-browser-devtools-cli $SESSION figma compare-page-with-design \
  --figma-file-key "xxx" --figma-node-id "1:header" \
  --selector "header")
echo "Header: $HEADER_RESULT"

# Compare hero section
HERO_RESULT=$(ironbee-browser-devtools-cli $SESSION figma compare-page-with-design \
  --figma-file-key "xxx" --figma-node-id "1:hero" \
  --selector ".hero-section")
echo "Hero: $HERO_RESULT"

# Compare full page
PAGE_RESULT=$(ironbee-browser-devtools-cli $SESSION figma compare-page-with-design \
  --figma-file-key "xxx" --figma-node-id "1:page" \
  --full-page)
echo "Full page: $PAGE_RESULT"

# Screenshot for reference
ironbee-browser-devtools-cli $SESSION content take-screenshot --name "implementation" --full-page
```
...
CLI="ironbee-browser-devtools-cli --json --quiet --session-id design-ci-$$"
...
ironbee-browser-devtools-cli interaction resize-viewport --width 1440 --height 900
```
