...
```bash
ironbee-browser-devtools-cli stub intercept-http-request --pattern <pattern> [options]
```
...
```bash
ironbee-browser-devtools-cli stub mock-http-response --pattern <pattern> [options]
```
...
```bash
ironbee-browser-devtools-cli stub clear [options]
```
...
```bash
# Clear specific stub
ironbee-browser-devtools-cli stub clear --stub-id "stub-1"
...
ironbee-browser-devtools-cli --json $SESSION stub list

# Test UI with mocked data
ironbee-browser-devtools-cli $SESSION content take-screenshot --name "mocked-data"
...
ironbee-browser-devtools-cli $SESSION interaction click --selector "#refresh-data"
ironbee-browser-devtools-cli $SESSION content take-screenshot --name "offline-state"
...
ironbee-browser-devtools-cli $SESSION stub intercept-http-request \
  --pattern "**/*" \
  --delay-ms 3000
...
ironbee-browser-devtools-cli $SESSION content take-screenshot --name "loading-state"

# Wait for completion
ironbee-browser-devtools-cli $SESSION sync wait-for-network-idle --timeout-ms 60000

# Screenshot after load
ironbee-browser-devtools-cli $SESSION content take-screenshot --name "loaded-state"
