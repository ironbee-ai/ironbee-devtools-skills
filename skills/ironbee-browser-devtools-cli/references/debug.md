...
```bash
ironbee-browser-devtools-cli debug put-tracepoint \
  --url-pattern "app.js" \
  --line-number 42
...
# With column (1-based)
ironbee-browser-devtools-cli debug put-tracepoint --url-pattern "app.js" --line-number 42 --column-number 8
```
...
ironbee-browser-devtools-cli debug list-probes
...
ironbee-browser-devtools-cli debug remove-probe --type tracepoint --id "tp_abc123"
ironbee-browser-devtools-cli debug remove-probe --type logpoint --id "lp_xyz"
ironbee-browser-devtools-cli debug remove-probe --type watch --id "w_abc"

# Clear probes (tracepoints, logpoints, watches). Omit types to clear all.
ironbee-browser-devtools-cli debug clear-probes
ironbee-browser-devtools-cli debug clear-probes --types tracepoint,logpoint
...
ironbee-browser-devtools-cli --json debug get-probe-snapshots --types tracepoint

# Filter by probe id
ironbee-browser-devtools-cli --json debug get-probe-snapshots --probe-id "tp_abc123"

# Polling: new snapshots since sequence 50
ironbee-browser-devtools-cli --json debug get-probe-snapshots --from-sequence 50

# Limit results
ironbee-browser-devtools-cli --json debug get-probe-snapshots --limit 10
...
ironbee-browser-devtools-cli --json debug get-probe-snapshots --max-call-stack-depth 20 --include-scopes '["local","closure"]' --max-variables-per-scope 50

# Clear snapshots (optional types and probeId)
ironbee-browser-devtools-cli debug clear-probe-snapshots
ironbee-browser-devtools-cli debug clear-probe-snapshots --types tracepoint,exceptionpoint
ironbee-browser-devtools-cli debug clear-probe-snapshots --probe-id "tp_abc123"
```
...
```bash
# Simple value
ironbee-browser-devtools-cli debug put-logpoint \
  --url-pattern "app.js" \
  --line-number 42 \
...
ironbee-browser-devtools-cli debug put-exceptionpoint --state none
```
...
# Remove one watch
ironbee-browser-devtools-cli debug remove-probe --type watch --id "w_abc123"
...
```bash
ironbee-browser-devtools-cli debug resolve-source-location \
  --url "http://localhost:3000/bundle.js" \
  --line 100

# With column (1-based)
ironbee-browser-devtools-cli debug resolve-source-location --url "file:///dist/app.js" --line 42 --column 8
```
...
```bash
ironbee-browser-devtools-cli --json debug status
```
...
# Get snapshots
ironbee-browser-devtools-cli $SESSION --json debug get-probe-snapshots --types tracepoint
```
...
```bash
ironbee-browser-devtools-cli debug put-exceptionpoint --state uncaught
ironbee-browser-devtools-cli navigation go-to --url "http://localhost:3000/buggy"
ironbee-browser-devtools-cli --json debug get-probe-snapshots --types exceptionpoint
```
